          ...
          >
            <div className="max-w-[62ch] text-[clamp(16px,1.15vw,18px)] leading-[1.68]">
              <Reveal>
                ...
                <p className="mb-5">
                  We wilden geen merk bouwen dat je haar behandelt als een probleem dat je met
                  steeds meer flesjes moet oplossen. Dus draaiden we het om: welke ene formule
                  doet echt iets, zonder de rest erbij te verkopen? Dat werd {PRODUCT.name}.
                </p>
              </Reveal>
              <Reveal delay={0.08}>
                <h3 className="display mb-3.5 mt-11 text-[clamp(20px,2.2vw,28px)]">
                  Wat we niet doen
                </h3>
                <p className="mb-5">
                  Geen dierproeven. Geen siliconen die na drie dagen ophopen. Geen
                  navulverpakking die duurder is dan de pot zelf. En geen ingrediëntenlijst
                  waarop het werkzame bestanddeel op plek negentien staat.
                </p>
              </Reveal>
              <Reveal delay={0.14}>
                <h3 className="display mb-3.5 mt-11 text-[clamp(20px,2.2vw,28px)]">
                ...
                <p className="mb-5">
                  Elke batch wordt met de hand gemengd, genummerd en gedateerd. Als een batch
                  niet klopt, gaat 'ie niet de deur uit. Dat betekent soms dat iets twee weken
                  uitverkocht is. Dat vinden we een beter probleem dan het alternatief.
                </p>
              </Reveal>
            </div>
          ...
          <div className="border-t border-ink/25">
            {TIMELINE.map(([when, text], i) => (
              <Reveal key={when} delay={i * 0.05}>
                <div className="group grid grid-cols-[40px_80px_1fr] items-baseline gap-5 border-b border-ink/15 py-5 transition-[border-color] duration-500 ease-folea hover:border-ink/40">
                  <span className="text-[11px] font-semibold tracking-[0.14em] opacity-40 transition-opacity duration-300 ease-folea group-hover:opacity-90 group-hover:text-flush-deep">
                    {String(i + 1).padStart(2, "0")}
                  </span>
                  <span className="font-display text-[14px] font-extrabold tracking-[0.06em]">
                    {when}
                  </span>
                  <p className="m-0 text-[15px] leading-snug opacity-80 transition-transform duration-500 ease-folea group-hover:translate-x-1">
                    {text}
                  </p>
                </div>
              </Reveal>
